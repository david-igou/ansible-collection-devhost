...
#
# mise's aqua backend already pinned a sha256 of the zip in mise.lock, but does
# NOT verify HashiCorp's GPG signature against the pinned release key. This hook
# restores the trust chain: pinned-fingerprint key → detached signature on
# SHA256SUMS → SHA256SUMS contains the zip's verified hash.
#
