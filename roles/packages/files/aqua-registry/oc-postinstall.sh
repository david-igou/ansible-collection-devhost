...
#
# mise's http backend already pinned a blake3 checksum of the downloaded tarball
# in mise.lock. This hook adds Red Hat's release signing key trust anchor
# (fingerprint 567E347AD0044ADE55BA8A5F199E2F91FD431D51).
#
# Re-fetches sha256sum.txt.gpg (an inline-signed OpenPGP message), verifies the
# GPG signature against the pinned-fingerprint key, then asserts the tarball mise
# pulled matches the GPG-signed sha256. Any failure exits non-zero → mise marks
# the install failed.
#
...
# signature). --decrypt extracts the plaintext AND verifies the signature in one
# step; exit 0 means the signature was good and signed by our anchored key.
